...
// ─── SPLIT ARCHITECTURE ──────────────────────────────────────────────────────
// Static context: field metadata, concept registry, statuses, vocab, tags/contexts
// → sent with cache_control, only changes when these data change
//
// Dynamic context: messages, task snapshot, completions, meetings
// → sent without caching, changes every call

export interface KarlStaticContext {
...
export interface KarlDynamicContext {
...
export interface KarlContextBundleSplit {
  static: KarlStaticContext;
  dynamic: KarlDynamicContext;
}

// Legacy bundle for backwards compatibility (deep context)
...
async function buildConceptRegistry(user_id: string): Promise<ConceptEntry[]> {
  ...
  const { data: koUser } = await db.from('ko_user').select('implementation_type').eq('id', user_id).maybeSingle();
...
async function buildFieldKnowledge(user_id: string): Promise<string> {
  ...
  return Object.entries(byType).map(([type, fieldLines]) => `${type}:\n${fieldLines.join('\n')}`).join('\n\n');
}

// ─── BUILD STATIC CONTEXT (CACHEABLE) ─────────────────────────────────────────

export async function buildKarlStaticContext(user_id: string): Promise<KarlStaticContext> {
  const db = createSupabaseAdmin();

  const [tagRes, contextRes, vocabRes, taskStatusRes, conceptRegistry] = await Promise.all([
    db.from('tag').select('name').eq('user_id', user_id).order('name'),
    db.from('context').select('context_id, name').eq('user_id', user_id).eq('is_archived', false).eq('is_visible', true).order('name'),
    db.from('karl_vocab').select('vocab_id, phrase, intent, object_type, use_count, rule_data, match, confirm, last_used').eq('user_id', user_id).eq('is_active', true).order('use_count', { ascending: false }).limit(100),
    db.from('task_status').select('task_status_id, label, display_order').eq('user_id', user_id).eq('is_active', true).order('display_order'),
    buildConceptRegistry(user_id),
  ]);

  const availableTags = tagRes.data?.length ? tagRes.data.map(t => t.name).join(', ') : 'none';
  const availableContexts = contextRes.data?.length ? contextRes.data.map(c => `${c.name}|${c.context_id}`).join(', ') : 'none';
  const taskStatuses = taskStatusRes.data?.length ? taskStatusRes.data.map(s => `  ${s.label} → ${s.task_status_id}`).join('\n') : 'none';
  ...
  const vocab = vocabRes.data?.length
    ? vocabRes.data.map(v => {
        ...
        if (v.rule_data) {
          ...
          const actions = (rule.actions ?? []).map((a: any) => `${a.field} ${a.mode} ${Array.isArray(a.value) ? a.value.join(', ') : a.value}`).join(' | ');
  ...
  return { fieldKnowledge, conceptRegistry, taskStatuses, vocab, availableTags, availableContexts };
}

// ─── BUILD DYNAMIC CONTEXT (NEVER CACHED) ─────────────────────────────────────

export async function buildKarlDynamicContext(user_id: string, context_filter: string | null = null): Promise<KarlDynamicContext> {
  const db = createSupabaseAdmin();

  const [situationRes, sessionRes, taskRes, obsRes, meetingRes, completionRes, extractRes, templateRes, contactRes, conceptRegistry] = await Promise.all([
    db.from('user_situation').select('brief, chat_history_depth, completion_window_days').eq('user_id', user_id).eq('is_active', true).maybeSingle(),
    db.from('ko_session').select('messages').eq('user_id', user_id).maybeSingle(),
    (() => {
      let q = db.from('task').select('task_id, title, bucket_key, tags, notes, sort_order, target_date, context_id, delegated_to').eq('user_id', user_id).eq('is_completed', false).eq('is_archived', false).order('sort_order', { ascending: true, nullsFirst: false }).order('created_at', { ascending: true });
      if (context_filter) q = q.eq('context_id', context_filter);
      return q;
    })(),
    db.from('karl_observation').select('content, observation_type').eq('user_id', user_id).eq('is_active', true).order('created_at', { ascending: false }).limit(MAX_OBSERVATIONS),
    db.from('meeting').select('meeting_id, title, meeting_date, attendees, tags, notes, outcome').eq('user_id', user_id).eq('is_completed', false).order('meeting_date', { ascending: false }).limit(15),
    db.from('completion').select('completion_id, title, completed_at, outcome, description, tags, context_id').eq('user_id', user_id).order('completed_at', { ascending: false }).limit(15),
    db.from('external_reference').select('external_reference_id, title, notes, description, created_at').eq('user_id', user_id).order('created_at', { ascending: false }).limit(15),
    db.from('document_template').select('document_template_id, name, doc_type, description, prompt_template, is_active').eq('user_id', user_id).eq('is_active', true).order('created_at', { ascending: false }).limit(15),
    db.from('contact').select('contact_id, name, email, primary_contact_method, contact_method_detail, notes').eq('user_id', user_id).eq('is_archived', false).order('name', { ascending: true }).limit(20),
    buildConceptRegistry(user_id),
  ]);
  ...
  const getObjectIcon = (key: string): string => conceptRegistry.find(c => c.concept_key === key && c.concept_type === 'object')?.icon ?? '';
  ...
  // Bucket snapshot
  const byBucket: Record<string, { task_id: string; title: string; tags: string[]; notes: string | null; target_date: string | null; context_id: string | null; delegated_to: string | null }[]> = {};
  for (const t of taskRes.data ?? []) {
    if (!byBucket[t.bucket_key]) byBucket[t.bucket_key] = [];
    byBucket[t.bucket_key].push({ task_id: t.task_id, title: t.title, tags: t.tags ?? [], notes: t.notes ?? null, target_date: t.target_date ?? null, context_id: t.context_id ?? null, delegated_to: t.delegated_to ?? null });
  }
  ...
  const { data: recentCompletionData } = await db.from('completion').select('completion_id, title, completed_at').eq('user_id', user_id).gte('completed_at', windowStart.toISOString()).order('completed_at', { ascending: false }).limit(20);
  ...
  if (contacts.length) {
    ...
    contacts.forEach((c, i) => {
      ...
      if (c.primary_contact_method) fcLines.push(`    contact via: ${c.primary_contact_method}${c.contact_method_detail ? ` (${c.contact_method_detail})` : ''}`);
      if (c.notes) fcLines.push(`    notes: ${trunc(c.notes, 300)}`);
    });
  }

  const fcSnapshot = fcLines.join('\n') || 'no FC objects';
  const observations = obsRes.data?.length ? obsRes.data.map(o => `[${o.observation_type}] ${o.content}`).join('\n') : '';

  return { situationBrief, recentMessages, bucketSnapshot, recentCompletions, meetingSnapshot, fcSnapshot, observations };
}

// ─── BUILD SPLIT CONTEXT ──────────────────────────────────────────────────────

export async function buildKarlContextSplit(user_id: string, context_filter: string | null = null): Promise<KarlContextBundleSplit> {
  const [staticCtx, dynamicCtx] = await Promise.all([
    buildKarlStaticContext(user_id),
    buildKarlDynamicContext(user_id, context_filter),
  ]);
  return { static: staticCtx, dynamic: dynamicCtx };
}

// ─── FORMAT SPLIT CONTEXT FOR PROMPT ──────────────────────────────────────────

export function formatStaticContextForPrompt(ctx: KarlStaticContext): string {
  const parts: string[] = [];
  ...
  if (ctx.taskStatuses && ctx.taskStatuses !== 'none') {
...
\n${formatConceptRegistry(ctx.conceptRegistry)}`);
  }

  return parts.join('\n\n');
}

export function formatDynamicContextForPrompt(ctx: KarlDynamicContext): string {
  const parts: string[] = [];

  if (ctx.situationBrief) {
...
export function formatContextForPrompt(bundle: KarlContextBundle): string {
  return formatStaticContextForPrompt(bundle) + '\n\n' + formatDynamicContextForPrompt(bundle);
}
...
export async function buildKarlDeepContext(user_id: string, context_filter: string | null = null): Promise<KarlDeepBundle> {
  ...
  const { data: situation } = await db.from('user_situation').select('completion_window_days').eq('user_id', user_id).eq('is_active', true).maybeSingle();
  const completionWindow = situation?.completion_window_days ?? 7;
  const windowStart = new Date();
  windowStart.setDate(windowStart.getDate() - completionWindow);

  const { data: completions } = await db.from('completion').select('title, outcome, completed_at, tags, context_id').eq('user_id', user_id).gte('completed_at', windowStart.toISOString()).order('completed_at', { ascending: false });
  ...
  const fullCompletions = completions?.length
    ? completions.map(c => `${completionIcon} [${c.completed_at?.slice(0, 10)}] ${c.title}` + (c.outcome ? `\n  Outcome: ${c.outcome}` : '') + (c.tags?.length ? `\n  Tags: ${c.tags.join(', ')}` : '')).join('\n\n')
    : 'no completions in window';

  const { data: tasks } = await db.from('task').select('title, bucket_key, tags, context:context_id(name)').eq('user_id', user_id).eq('is_completed', false).eq('is_archived', false).neq('bucket_key', 'capture');
  ...
  const tasksByContext = Object.entries(byContext).map(([ctx, items]) => `${ctx}:\n${items.join('\n')}`).join('\n\n') || 'no curated tasks';
...
export async function writeKarlObservation(user_id: string, content: string, observation_type: 'pattern' | 'preference' | 'flag' = 'pattern', tags: string[] = []): Promise<void> {
  const db = createSupabaseAdmin();
  const { count } = await db.from('karl_observation').select('observation_id', { count: 'exact', head: true }).eq('user_id', user_id).eq('is_active', true);
  if ((count ?? 0) >= MAX_OBSERVATIONS) {
    const { data: oldest } = await db.from('karl_observation').select('observation_id').eq('user_id', user_id).eq('is_active', true).order('created_at', { ascending: true }).limit(1).maybeSingle();
    if (oldest) await db.from('karl_observation').update({ is_active: false }).eq('observation_id', oldest.observation_id);
  }
  await db.from('karl_observation').insert({ user_id, content, observation_type, tags });
}

export async function updateFieldLlmNotes(user_id: string, object_type: string, field: string, llm_notes: string): Promise<void> {
  const db = createSupabaseAdmin();
  const { error } = await db.from('ko_field_metadata').update({ llm_notes }).eq('user_id', user_id).eq('object_type', object_type).eq('field', field);
  if (error) console.error('[updateFieldLlmNotes]', error);
  else console.log(`[updateFieldLlmNotes] updated ${object_type}.${field}`);
}

export async function upsertKarlVocab(user_id: string, phrase: string, intent: string, object_type: string): Promise<void> {
  const db = createSupabaseAdmin();
  const normalised = phrase.toLowerCase().trim();
  const { data: existing } = await db.from('karl_vocab').select('vocab_id, use_count').eq('user_id', user_id).eq('phrase', normalised).maybeSingle();
  if (existing) {
    await db.from('karl_vocab').update({ use_count: existing.use_count + 1, last_used: new Date().toISOString() }).eq('vocab_id', existing.vocab_id);
  } else {
    await db.from('karl_vocab').insert({ user_id, phrase: normalised, intent, object_type, use_count: 1 });
  }
}

export async function writeKarlVocabRule(user_id: string, phrase: string, description: string, rule_data: Record<string, any>, match: 'contains' | 'exact' | 'starts_with' = 'contains', confirm: boolean = true): Promise<{ vocab_id: string } | null> {
  const db = createSupabaseAdmin();
  const normalised = phrase.toLowerCase().trim();
  const { data: existing } = await db.from('karl_vocab').select('vocab_id').eq('user_id', user_id).eq('phrase', normalised).maybeSingle();
  if (existing) {
    await db.from('karl_vocab').update({ rule_data, match, confirm, description, last_used: new Date().toISOString() }).eq('vocab_id', existing.vocab_id);
    return { vocab_id: existing.vocab_id };
  }
  const { data } = await db.from('karl_vocab').insert({ user_id, phrase: normalised, intent: 'rule', object_type: rule_data.applies_to ?? 'task', rule_data, match, confirm, use_count: 0 }).select('vocab_id').single();
...
export async function touchKarlVocabRule(user_id: string, vocab_id: string): Promise<void> {
  ...
  await db.from('karl_vocab').update({ use_count: (existing?.use_count ?? 0) + 1, last_used: new Date().toISOString() }).eq('vocab_id', vocab_id).eq('user_id', user_id);
}

export async function loadActiveVocabRules(user_id: string): Promise<Array<{ vocab_id: string; phrase: string; match: string; confirm: boolean; rule_data: Record<string, any> }>> {
  const db = createSupabaseAdmin();
  const { data } = await db.from('karl_vocab').select('vocab_id, phrase, match, confirm, rule_data').eq('user_id', user_id).eq('is_active', true).not('rule_data', 'is', null).order('use_count', { ascending: false });
  return (data ?? []).filter(r => r.rule_data) as any;
}

export async function appendSessionMessage(user_id: string, role: 'user' | 'karl', content: string): Promise<void> {
  const db = createSupabaseAdmin();
  const { data: session } = await db.from('ko_session').select('ko_session_id, messages').eq('user_id', user_id).maybeSingle();
  if (!session) return;
  const { data: situation } = await db.from('user_situation').select('chat_history_depth').eq('user_id', user_id).eq('is_active', true).maybeSingle();
