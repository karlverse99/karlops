...
export async function routeCommand(
  ...
  try {
    ...
    const staticSystemPrompt = [
      ...
      'Must include: name, description, output_format (md), prompt_template (formatting instructions; NO {{placeholders}}).',
      'Whenever the report pulls workspace data, also include:',
      '  - selected_elements: string[] of "object_type.field" keys (same as Templates → Data Elements), e.g. completion.title, completion.completed_at, completion.outcome.',
      '  - element_filters: object whose keys are the SAME "object_type.field" strings as selected_elements; values are filter inputs (numbers, strings, string[], booleans) per field metadata — e.g. a numeric window on completions is stored on the element key that represents that filter in the UI.',
      'Passthrough templates (no DB pulls): selected_elements: [] and element_filters: {}.',
      'Optional: user_prompt_additions, template_mode (default karl), filename_suffix_format, context_id, sections[] (legacy), tags[].',
      '',
