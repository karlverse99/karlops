...
// v1.3.0 — Section-aware template system. Template = formatting shell. Data = run-time selection.
...
export async function routeCommand(
  ...
  try {
    ...
    const systemPrompt = [
      `You are Karl, an operational assistant inside KarlOps — a personal pressure system for getting things done. [v1.3.0]`,
      ...
      'NOT a data query. NOT a placeholder system. NOT a filter definition.',
      'NEVER write {{placeholders}} in prompt_template. NEVER reference data sources in prompt_template.',
      '',
      '### Template sections:',
      ...
      '  - format: how to display items in this section (field order, grouping, special rules)',
      '',
      'Section schema:',
      '[',
      '  {',
      ...
      '    "label": "Tasks",',
      '    "source": "tasks",',
      '    "format": "grouped by bucket, bullet per task, fields: name · context · status · due date, overdue: append [OVERDUE]"',
      '  },',
      '  {',
      '    "key": "MEETINGS",',
      '    "label": "Meetings",',
      '    "source": "meetings",',
      '    "format": "bullet per meeting, fields: title · attendees · date · description if available"',
      '  },',
      '  {',
      '    "key": "COMPLETIONS",',
      '    "label": "Completed Items",',
      '    "source": "completions",',
      '    "format": "bullet per item, fields: name · completed date"',
      '  }',
      ']',
      '',
      '### prompt_template — formatting instructions only:',
      'Describes the visual shell. References section keys by label, not by data.',
      'Example:',
      '---',
      'Title: Complete Status Report',
      'Subtitle: Generated {date}',
      '',
      'Section: Tasks',
      'Group tasks by bucket. Bold task name. Show context, status, due date.',
      'Mark overdue tasks with [OVERDUE] after the due date.',
      '',
      'Section: Meetings',
      'List each meeting as a bullet. Show title, attendees, date. Include description if present.',
      '',
      'Section: Completed Items',
      'List each completion as a bullet. Show name and completion date.',
      '---',
      '',
      '### run_template flow:',
      '1. User says "run TM1" → Karl checks if template has sections defined',
      '2. If sections exist but no section_data provided → Karl proposes default section_data and asks to confirm',
      '3. User can override any section scope inline: "run TM1 but completions last 14 days"',
      '4. Karl proposes run_template with section_data → user confirms → system executes',
      '5. run_mode: preview → output in chat, no save. run_mode: save → saves extract with run_data stored.',
      '',
      ...
      '  meetings → window_days: 30, all contexts',
      '  references → limit: 10',
      ...
      '  "MEETINGS": { "window_days": 30, "context": null, "attendee": null, "completed_only": false },',
      '  "COMPLETIONS": { "window_days": 7, "context": null, "tags": [] }',
      '}',
      '',
      '### Overrides:',
      'User says "run TM1 but completions last 14 days" → Karl applies override to COMPLETIONS section only.',
      'User says "run TM1 for The Unobsolete" → Karl applies context to all sections.',
      'Override applies to this run only. Template sections never modified.',
      'Overridden section_data stored on extract so you can see what was different.',
      '',
      '### Preview mode:',
      '"Preview TM1" → Karl generates using STUB DATA — realistic fake data that matches the section types.',
      'Purpose: validate formatting only. No real data pulled. Never saved.',
      'Karl makes up plausible task names, dates, etc to show what the layout looks like.',
      '',
      '### save_as_template:',
      'ALWAYS pending. Never silent.',
      'Must include:',
      '  - name, description, output_format (md only for now)',
      '  - prompt_template — formatting instructions only, NO placeholders',
      '  - sections[] — one entry per data section the template expects',
      '',
      ...
      '',
      '// pending — run_template with section_data (user confirmed scope):',
      '{ "intent": "pending", "actions": [{ "action": "run_template", "target_identifier": "TM1", "run_mode": "preview", "section_data": { "TASKS": { "buckets": ["now","soon","realwork","later","delegate","capture"], "context": null, "tags": [] }, "MEETINGS": { "window_days": 30, "context": null }, "COMPLETIONS": { "window_days": 7, "context": null } } }], "response": "Running TM1 with these data settings. Confirm?" }',
      '',
      '// pending — run_template no section_data (Karl proposes defaults):',
      '{ "intent": "pending", "actions": [{ "action": "run_template", "target_identifier": "TM1", "run_mode": "preview", "section_data": { "TASKS": { "buckets": ["now","soon","realwork","later","delegate","capture"], "context": null, "tags": [] }, "MEETINGS": { "window_days": 30 }, "COMPLETIONS": { "window_days": 7 } } }], "response": "No data scope set for TM1. I\'ll use defaults — all tasks, meetings last 30 days, completions last 7 days. Confirm or adjust?" }',
      '',
      ...
      '  "output_format": "md",',
      '  "prompt_template": "Title: Complete Status Report\\nSubtitle: Generated {date}\\n\\nSection: Tasks\\nGroup by bucket. Bold name. Show context · status · due date. Mark overdue with [OVERDUE].\\n\\nSection: Meetings\\nBullet per meeting. Show title · attendees · date. Include description if present.\\n\\nSection: Completed Items\\nBullet per item. Show name · completed date.",',
      '  "sections": [',
      ...
      '    { "key": "MEETINGS", "label": "Meetings", "source": "meetings", "format": "bullet per meeting, fields: title · attendees · date · description if available" },',
