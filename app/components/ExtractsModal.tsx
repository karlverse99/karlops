...
function exportAsCSV(extracts: Extract[]): void {
  ...
  a.href = url; a.download = `extracts-${new Date().toISOString().slice(0, 10)}.csv`; a.click();
  URL.revokeObjectURL(url);
}

function exportAsMD(extracts: Extract[]): void {
  const lines = ['# Extracts', ''];
  ...
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url; a.download = `extracts-${new Date().toISOString().slice(0, 10)}.md`; a.click();
  URL.revokeObjectURL(url);
}
...
export default function ExtractsModal({ userId, accessToken, onClose, onCountChange }: ExtractsModalProps) {
  ...
  // ─── Drag/resize — no centered state, always absolute ──────────────────────
  const initX = Math.max(20, Math.round(window.innerWidth  / 2 - 500));
  const initY = Math.max(20, Math.round(window.innerHeight / 2 - 425));
  ...
  const loadExtracts = async () => {
    ...
    const { data } = await supabase
      ...
      .select(`external_reference_id, title, filename, location, ref_type, description, notes, tags, document_template_id,
        context:context_id ( name, context_id ),
        task:task_id ( title, task_id ),
        meeting:meeting_id ( title, meeting_id )`)
      .eq('user_id', userId)
      .order('created_at', { ascending: false });
    if (data) { setExtracts(data as any); onCountChange(data.length); }
    setLoading(false);
  };
  ...
  const loadTasks = async () => {
    const { data } = await supabase.from('task').select('task_id, title').eq('user_id', userId).eq('is_completed', false).eq('is_archived', false).order('title');
    if (data) setTasks(data);
  };

  const loadMeetings = async () => {
    const { data } = await supabase.from('meeting').select('meeting_id, title').eq('user_id', userId).eq('is_completed', false).order('title');
    if (data) setMeetings(data);
  };

  useEffect(() => {
    loadExtracts(); loadContexts(); loadTags(); loadFieldMeta(); loadTasks(); loadMeetings();
  }, []);
  ...
  useEffect(() => {
    const onMove = (e: MouseEvent) => {
      ...
      if (resizing.current) {
        setSize({
          w: Math.max(700, resizeStart.current.w + (e.clientX - resizeStart.current.mx)),
  ...
  const filtered = extracts.filter(r => {
  ...
  const openAdd = () => {
    ...
    setTagSearch(''); setSelectedGroupId(''); setErr(''); setRerunMsg('');
  ...
  const handleRerun = async () => {
    if (!selected?.document_template_id) return;
    setRerunning(true); setRerunMsg('');
    ...
    try {
      ...
      const res = await fetch('/api/ko/template/run', {
        ...
        body: JSON.stringify({ template_id: selected.document_template_id }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error ?? 'Run failed');

      // Build new filename with today's date
      const baseName   = (selected.filename ?? selected.title).replace(/-\d{4}-\d{2}-\d{2}\.md$/, '').replace(/\s—\s.+$/, '');
      const dateStr    = new Date().toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
      const dateSlug   = new Date().toISOString().slice(0, 10);
      const newTitle    = `${baseName} — ${dateStr}`;
      const newFilename = `${baseName.toLowerCase().replace(/\s+/g, '-')}-${dateSlug}.md`;

      // Save new extract record
      const { error } = await supabase.from('external_reference').insert({
        ...
        context_id:           selected.context?.context_id || null,
      ...
      await loadExtracts();
      setRerunMsg('✓ New extract saved.');

    } catch (err: any) {
      setRerunMsg(`Error: ${err.message}`);
    } finally {
      setRerunning(false);
    }
  };
  ...
  const renderField = (meta: FieldMeta) => {
    ...
    switch (meta.field) {
      ...
      case 'description':
        return <div key="description" style={{ marginBottom: '0.85rem' }}>{label}<textarea value={formDescription} onChange={e => setFormDescription(e.target.value)} rows={1} style={{ ...inputStyle, resize: 'vertical' } as any} onFocus={e => (e.target.style.borderColor = ACCENT)} onBlur={e => (e.target.style.borderColor = '#ddd')} /></div>;

      case 'notes':
        return <div key="notes" style={{ marginBottom: '0.85rem' }}>{label}<textarea value={formNotes} onChange={e => setFormNotes(e.target.value)} rows={3} style={{ ...inputStyle, resize: 'vertical', minHeight: '64px' } as any} onFocus={e => (e.target.style.borderColor = ACCENT)} onBlur={e => (e.target.style.borderColor = '#ddd')} /></div>;
        ...
        return (
          <div key="tags" style={{ marginBottom: '0.85rem' }}>
            ...
            <div style={{ display: 'flex', gap: '0.5rem' }}>
              <select value={selectedGroupId} onChange={e => setSelectedGroupId(e.target.value)} style={{ ...inputStyle, flex: '0 0 130px', fontSize: '0.72rem', padding: '0.35rem 0.5rem' } as any}>
                <option value="">All groups</option>
                {tagGroups.map(g => <option key={g.tag_group_id} value={g.tag_group_id}>{g.name}</option>)}
              </select>
              <div style={{ position: 'relative', flex: 1 }}>
                <input value={tagSearch} onChange={e => { setTagSearch(e.target.value); setShowTagDrop(true); }} onFocus={() => setShowTagDrop(true)} onBlur={() => setTimeout(() => setShowTagDrop(false), 150)} placeholder="Search tags..." style={{ ...inputStyle, marginBottom: 0 }} onFocus={e => (e.target.style.borderColor = ACCENT)} onBlur={e => (e.target.style.borderColor = '#ddd')} />
      ...
      case 'context_id':
        return (
          <div key="context_id" style={{ marginBottom: '0.85rem' }}>{label}
            <select value={formContextId} onChange={e => setFormContextId(e.target.value)} style={{ ...inputStyle, cursor: 'pointer' } as any} onFocus={e => (e.target.style.borderColor = ACCENT)} onBlur={e => (e.target.style.borderColor = '#ddd')}>
      ...
      case 'task_id':
        return (
          <div key="task_id" style={{ marginBottom: '0.85rem' }}>{label}
            <select value={formTaskId} onChange={e => setFormTaskId(e.target.value)} style={{ ...inputStyle, cursor: 'pointer' } as any} onFocus={e => (e.target.style.borderColor = ACCENT)} onBlur={e => (e.target.style.borderColor = '#ddd')}>
              <option value="">— none —</option>
      ...
      case 'meeting_id':
        return (
          <div key="meeting_id" style={{ marginBottom: '0.85rem' }}>{label}
            <select value={formMeetingId} onChange={e => setFormMeetingId(e.target.value)} style={{ ...inputStyle, cursor: 'pointer' } as any} onFocus={e => (e.target.style.borderColor = ACCENT)} onBlur={e => (e.target.style.borderColor = '#ddd')}>
              <option value="">— none —</option>
      ...
      case 'document_template_id':
        return null; // surfaced via rerun button, not a form field
    ...
    <div style={{ width: '340px', flexShrink: 0, display: 'flex', flexDirection: 'column', borderRight: `1px solid ${ACCENT_BORDER}`, height: '100%' }}>
      <div style={{ padding: '0.75rem', borderBottom: `1px solid ${ACCENT_BORDER}`, display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
        ...
        <div style={{ display: 'flex', gap: '0.4rem' }}>
          <select value={filterContext} onChange={e => setFilterContext(e.target.value)} style={{ ...inputStyle, flex: 1, fontSize: '0.7rem', padding: '0.3rem 0.5rem' } as any}>
            <option value="">All contexts</option>
            {contexts.map(c => <option key={c.context_id} value={c.context_id}>{c.name}</option>)}
          </select>
          <select value={filterTag} onChange={e => setFilterTag(e.target.value)} style={{ ...inputStyle, flex: 1, fontSize: '0.7rem', padding: '0.3rem 0.5rem' } as any}>
            <option value="">All tags</option>
            {allTags.map(t => <option key={t.tag_id} value={t.name}>{t.name}</option>)}
          </select>
        </div>
        <div style={{ color: '#999', fontSize: '0.65rem', fontFamily: 'monospace' }}>{filtered.length} of {extracts.length}</div>
      </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        {loading ? (
          ...
          <div style={{ color: '#bbb', fontSize: '0.75rem', padding: '1rem', fontFamily: 'monospace' }}>No extracts found.</div>
        ) : (
          filtered.map((r, idx) => {
            const isSelected = selected?.external_reference_id === r.external_reference_id;
            const identifier = `EX${idx + 1}`;
            const isGenerated = r.document_template_id !== null;
            return (
              ...
              >
                ...
                {r.filename && <span style={{ color: ACCENT, fontSize: '0.6rem', flexShrink: 0, fontFamily: 'monospace', opacity: 0.7 }}>{r.filename}</span>}
                ...
                {isGenerated && <span style={{ fontSize: '0.58rem', color: ACCENT, background: ACCENT_BG, border: `1px solid ${ACCENT_BORDER}`, borderRadius: 2, padding: '0.05rem 0.3rem', flexShrink: 0 }}>gen</span>}
              </div>
            );
          })
        )}
  ...
  const renderRight = () => {
    if (mode === 'empty') return (
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#ccc', fontSize: '0.8rem', fontFamily: 'monospace', flexDirection: 'column', gap: '0.5rem' }}>
        ...
        <div>Select an extract to edit</div>
      </div>
    );
    ...
    const identifier = !isAdd ? `EX${extracts.findIndex(r => r.external_reference_id === editId) + 1}` : null;
    const isGenerated = !isAdd && selected?.document_template_id;
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '1.25rem', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1rem' }}>
          <div style={{ color: '#888', fontSize: '0.7rem', fontFamily: 'monospace', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
            {isAdd ? 'New Extract' : `Editing ${identifier}`}
          </div>
          {isGenerated && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              {rerunMsg && (
                <span style={{ fontSize: '0.7rem', color: rerunMsg.startsWith('✓') ? '#10b981' : '#ef4444', fontFamily: 'monospace' }}>{rerunMsg}</span>
              )}
              <button onClick={handleRerun} disabled={rerunning}
                style={{ background: ACCENT, border: 'none', color: '#fff', padding: '0.3rem 0.75rem', borderRadius: 4, fontSize: '0.72rem', fontFamily: 'monospace', cursor: rerunning ? 'not-allowed' : 'pointer', fontWeight: 600, opacity: rerunning ? 0.6 : 1 }}>
        ...
        {/* Location quick-open */}
        {!isAdd && selected?.location && selected.location !== 'generated' && (
        ...
        {/* Generated badge */}
        {isGenerated && (
          <div style={{ padding: '0.4rem 0.7rem', background: ACCENT_BG, border: `1px solid ${ACCENT_BORDER}`, borderRadius: 4, color: ACCENT, fontSize: '0.7rem', fontFamily: 'monospace', marginBottom: '1rem' }}>
            Generated extract · hit rerun to create a fresh version with today's data
          </div>
        )}
        ...
        <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'flex-end', marginTop: 'auto', paddingTop: '1rem' }}>
          ...
          <button onClick={handleSave} disabled={saving} style={{ background: ACCENT, border: `1px solid ${ACCENT}`, color: '#fff', padding: '0.4rem 0.8rem', borderRadius: '4px', fontFamily: 'monospace', fontSize: '0.75rem', cursor: 'pointer', fontWeight: 600 }}>{saving ? '...' : isAdd ? 'save extract' : 'save changes'}</button>
        </div>
      </div>
    );
  };
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 100, pointerEvents: 'none' }}>
      <div style={{ position: 'absolute', left: pos.x, top: pos.y, width: size.w, height: size.h, background: '#ffffff', border: `2px solid ${ACCENT}`, borderRadius: '8px', display: 'flex', flexDirection: 'column', fontFamily: 'monospace', boxShadow: '0 20px 60px rgba(0,0,0,0.3)', overflow: 'hidden', pointerEvents: 'all' }}>
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
            ...
            <span style={{ color: '#000', fontSize: '0.72rem', opacity: 0.5 }}>{extracts.length} total</span>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
            ...
            <button onClick={openAdd}
              ...
              onMouseEnter={e => (e.currentTarget.style.background = '#222')} onMouseLeave={e => (e.currentTarget.style.background = '#000')}
            >+ new extract</button>
            <button onClick={onClose} style={{ background: 'none', border: 'none', color: '#000', cursor: 'pointer', fontSize: '1rem', lineHeight: 1, opacity: 0.5, pointerEvents: 'all' }} onMouseEnter={e => (e.currentTarget.style.opacity = '1')} onMouseLeave={e => (e.currentTarget.style.opacity = '0.5')}>✕</button>
          </div>
        </div>
