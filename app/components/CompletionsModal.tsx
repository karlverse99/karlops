...
export default function CompletionsModal({ userId, accessToken, onClose, onCountChange }: CompletionsModalProps) {
  ...
  const filtered = completions.filter(c => {
    ...
    if (filterDateRange !== 'all') {
      const d = new Date(c.completed_at); const now = new Date();
          ...
          filtered.map((c) => {
            ...
            return (
              ...
              >
                ...
                <span style={{ color: ACCENT, fontSize: '0.7rem', flexShrink: 0, fontFamily: 'monospace', fontWeight: 500 }}>{formatDate(c.completed_at)}</span>
                <span style={{ color: '#111', fontSize: '0.82rem', flex: 1, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', fontFamily: 'monospace', fontWeight: 500 }}>{c.title}</span>
              </div>
            );
          })
        )}
  ...
  const renderRight = () => {
    if (mode === 'empty') return (
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#ccc', fontSize: '0.8rem', fontFamily: 'monospace', flexDirection: 'column', gap: '0.5rem' }}>
        ...
        <div>Select a completion to edit</div>
      </div>
    );
    ...
    return (
      <div style={{ flex: 1, overflowY: 'auto', padding: '1.25rem', display: 'flex', flexDirection: 'column' }}>
        <div style={{ color: '#888', fontSize: '0.7rem', fontFamily: 'monospace', marginBottom: '1rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
          {isAdd ? 'New Completion' : `Editing CM${completions.findIndex(c => c.completion_id === editId) + 1}`}
        </div>
